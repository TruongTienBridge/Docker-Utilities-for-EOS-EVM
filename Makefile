stop-node:
	docker rm -f eos_evm_container

start-node: stop-node
	docker run -d -p 8888:8888 -p 9981:9981 eedu:latest
...
boostrap-system:
	...
	cleos create account eosio eosio.rex EOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV EOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
	cleos set contract eosio . /app/reference-contracts/build/contracts/eosio.boot/eosio.boot.wasm /app/reference-contracts/build/contracts/eosio.boot/eosio.boot.abi
	...
	cleos set contract eosio.msig . /app/reference-contracts/build/contracts/eosio.msig/eosio.msig.wasm /app/reference-contracts/build/contracts/eosio.msig/eosio.msig.abi
	cleos set contract eosio.token . /app/reference-contracts/build/contracts/eosio.token/eosio.token.wasm /app/reference-contracts/build/contracts/eosio.token/eosio.token.abi
	until cleos set contract eosio . /app/reference-contracts/build/contracts/eosio.system/eosio.system.wasm /app/reference-contracts/build/contracts/eosio.system/eosio.system.abi ; do \
		sleep 3s ; \
	done
...
setup-evm-contract:
	cleos system newaccount eosio eosio.evm EOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV EOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV --stake-net "100.0000 EOS" --stake-cpu "100.0000 EOS" --buy-ram-bytes 10598000
	until cleos set contract eosio.evm . /app/custom/evm_runtime/evm_runtime.wasm /app/custom/evm_runtime/evm_runtime.abi ; do \
		sleep 3s ; \
	done
	cleos push action eosio.evm init '[15555, [150000000000, 10000, "0.0100 EOS"]]' -p eosio.evm
...
build-docker-dev:
	docker build -t eedu_dev -f Dockerfile.dev .